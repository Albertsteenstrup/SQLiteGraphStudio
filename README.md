...
**The newest migration is the default.** The picker offers any other version before opening, and once open, a control in the pane header steps through the history — arrows for one migration at a time, a menu for any point in it, and **Database → Replay Migrations Through** for the same choice from the menu bar. Graph layout, notes, clusters, stories and saved queries belong to the migration set, not to one revision of it, so they survive stepping between versions.
...
This model is **structure only**: there are no rows to browse, the SQL runner is unavailable, and every editing action is refused with an explanation. Notes, cluster hints, stories and the AI skills all still work, since they live in a `<folder>.studio.json` sidecar next to the migration folder.
...
- The compact graph toolbar keeps search and **Filter** visible. The **Graph options (…)** menu contains **Graph visuals** (the same switches as **View ▸ Graph Visuals**), node size, stories, relayout, and table counts; active filters never add another toolbar row.
