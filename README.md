...
At full-model zoom, authored group titles and a few optional enlarged `overviewTables` nodes provide orientation while the other nodes stay compact. The names stay inside the nodes, with no floating labels or leader lines. A coding agent can then show a readable subset spanning the main domains and move into a narrower group or table. The sidecar hints guide that presentation; they do not restrict the agent to those tables.
