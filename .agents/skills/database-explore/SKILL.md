...
- For row questions, use `studio_open_table`, `studio_configure_table`, `studio_fetch_rows`, `studio_inspect_value`, `studio_follow_record`, or a read-only query. Return only relevant bounded values to the agent. Make the filter, sort, and SQL inspectable in the app. A displayed result is not necessarily a complete result set.
