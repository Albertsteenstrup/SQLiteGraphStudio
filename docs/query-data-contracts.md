...
- CSV uses unquoted `\N` for SQL NULL and quoted `"\N"` for literal text of that value. Empty text is quoted `""`; embedded delimiters, quotes and newlines follow standard CSV quoting. Binary values are base64.
...
- A streaming export writes a temporary sibling file. The destination is replaced atomically only after all rows finish and cancellation is checked. Failure or cancellation removes the temporary file and preserves any previous destination.
