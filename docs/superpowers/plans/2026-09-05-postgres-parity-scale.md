...
- [ ] Review and verify: independent spec review followed by code quality review; repair actionable findings. Run focused tests and `swift test --parallel --num-workers 1`, release build, real UI checks and PostgreSQL integration when a current local endpoint is available. Record full results and limitations.
