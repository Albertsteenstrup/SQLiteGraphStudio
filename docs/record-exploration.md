...
The graph shows explored relationships, not an exhaustive database scan. Limits are 100 nodes, 250 edges, depth 4, 120 query reservations per root, and 2 concurrent requests. FK pages contain at most25 records; mapped pages contain at most5 edges and reserve7 queries. Caps and additional pages remain visible. Collapse branches or select a new root to continue. Requests are cancellable; stale completions cannot replace a newer inspector. PostgreSQL record SELECTs also have a5-second transaction-local statement timeout. Offset pages are live reads, not a cross-request database snapshot; concurrent database changes can shift page membership.
