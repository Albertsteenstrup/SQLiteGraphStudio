...
Coding agents can discover these application-defined mappings with the read-only `studio_list_record_mappings` MCP tool. It returns the exact `mapping_id`, sidecar index provenance, current-schema validation status, table/column roles and bounded typed scope filters. After inspecting a record from the mapping's node table, the agent can pass that exact `mapping_id` to `studio_follow_record`. A sidecar mapping describes an application graph; it does not become a database-declared foreign key or a `relation_id`.
