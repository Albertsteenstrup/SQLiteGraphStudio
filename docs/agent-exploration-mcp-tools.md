...
| `studio_open_source` | Open or reconnect a named SQLite file, PostgreSQL connection document, supported backup, or saved schema artifact. Use an explicit or established source; return candidate choices if ambiguous. | Context, path/document/source reference, reuse policy | Source identity, engine, revision, capabilities, or opening/restore job. Uses existing app credential and restore flows; never returns secrets. |
...
| Tool | Agent-visible description | Main inputs | Result and effect |
| --- | --- | --- | --- |
| `studio_open_table` | Open a table or view in the workspace's data pane and coordinate its graph selection. Use when actual rows will help answer the question. | Workspace, table ID, optional record/filter context | Grid ID, column metadata, load job/readiness. Does not automatically return all displayed values to the agent. |
