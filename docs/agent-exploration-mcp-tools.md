...
| `studio_find_relations` | Find declared foreign-key neighbors around selected tables. Use the graph edge `id` for key focus and `record_relation_id`, when present, for record navigation. Application-only associations are not returned as declared edges. | Workspace, table IDs, hop count | Neighbor table IDs and declared edges. This build does not infer or rank paths. |
...
| `studio_arrange_tables` | Compact a subset, place one table at explicit coordinates, or relayout the graph. Disconnected tables may sit together without implying a relation. | Workspace, operation, table IDs for compact or one table ID plus x/y for position | Applied positions. Arbitrary multi-table position batches and persistent pinning are unavailable. |
...
| `studio_follow_record` | Follow a full inspected record through one exact declared relationship or saved application mapping. For a foreign key, pass `studio_find_relations`'s `record_relation_id` as `relation_id`; its graph-edge `id` is for graph focus and is different. For a sidecar mapping, first discover its exact ID with `studio_list_record_mappings` and pass that as `mapping_id`. A bounded cell slice has no stable record identity. | Current inspected record, record relation/mapping ID, direction, display intent | Bounded related records, page continuation, provenance and visible inspector state. No fabricated schema edge. |
| `studio_show_record_graph` | Display the current stable-identity record and optionally expand declared one-edge neighbors. Get exact record relation IDs from `studio_find_relations`; an empty relation list shows only the seed. | Workspace, optional matching seed record, up to eight declared record relation IDs, direction | Rendered record IDs, declared links, omissions and expansion controls. Arbitrary seeds and multi-hop expansion are unavailable. |
...
| `studio_wait_events` | Wait up to 10 seconds for selected-workspace view or current-presentation changes while orchestrating a live explanation. For a specific job or presentation, use `studio_get_job` or `studio_get_presentation`; ID-specific event filters are unavailable. | Context, workspace, `after_cursor`, bounded wait | Current view/presentation snapshot, cursor and timeout flag. This is not a durable ordered event stream and does not receive microphone audio. |
