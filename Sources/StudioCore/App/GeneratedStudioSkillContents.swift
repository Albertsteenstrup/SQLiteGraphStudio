...
extension StudioSkills {
    ...
    - Use `studio_show_tables`, `studio_select_objects`, `studio_expand_tables`, `studio_focus_keys`, `studio_arrange_tables`, and `studio_set_camera` as needed. `studio_show_tables` fits the visible set; for widely separated tables, use `studio_arrange_tables` with `operation=compact` to bring them together and refit. Use `studio_set_camera` with `mode=fit_visible` after an exploratory pan or zoom when a fresh fit would help. Moving disconnected tables together changes layout only. Check `studio_get_view` to confirm the actual visible set after the user's graph filter, and use `graph_visible` and `visible_pane` to check what is onscreen. `visual_state` acknowledges graph drawing, not table or query content. Preserve a path back to the full view.
